const CoupangCategoryHandler = (() => {
    ...
    function getCategory() {
        const breadcrumbEl = document.querySelector('#breadcrumb'); // #breadcrumb 요소 선택
        if (breadcrumbEl) {
            const categoryNames = Array.from(breadcrumbEl.querySelectorAll('li')) // li 태그들 선택
                .map((li) => li.innerText.trim()) // 각 li의 텍스트를 가져와서 배열로 저장
                .filter(Boolean); // 빈 값을 제거
            return categoryNames; // 카테고리 이름들을 배열로 반환
        }
        return []; // breadcrumb가 없으면 빈 배열 반환
    }
