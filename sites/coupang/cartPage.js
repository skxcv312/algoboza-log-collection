    ...
    rows.forEach((row) => {
      try {
        const nameEl = row.querySelector(
          "td.product-box .product-name-part a span"
        );
        const priceEl = row.querySelector("div.unit-total-sale-price");
        const deliveryEl = row.querySelector(
          "div.delivery-date-line span.arrive-date"
        );
        ...
        const productDetails = {
          ...
          discountPrice: priceEl
            ? parseInt(priceEl.innerText.replace(/[^0-9]/g, ""), 10)
            : 0,
