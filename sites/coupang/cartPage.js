    ...
    function extractInfo() {
        ...
        cartGoodsEl.forEach((el) => {
            ...
            const productDetails = {
                ...
                price: parseInt(el.querySelector('span[class="cart-item__price"]').innerText.replace(/[^0-9]/g, ""), 10),
                // 할인된 가격 (함수로 추출)
                discountPrice: getDiscountPrice(el),
                // 수량 (수량 정보를 담은 속성으로 바꿈)
                quantity: parseInt(el.querySelector('input[class="cart-item__quantity"]')?.value || 1),
                // 카테고리 정보 (쿠팡의 상품 카테고리 정보로 바꿈)
                category: goods.getAttribute("data-category")?.split("|") || [],
                // 브랜드명 (쿠팡에서 브랜드 정보를 가져오는 방식으로 수정)
                brand: el.querySelector('span[class="cart-item__brand"]')?.innerText || "브랜드 미제공",
            };
